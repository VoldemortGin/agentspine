...
src/agentspine/
  ...
  tools/tool.py             Tool 协议 + EchoTool / CalcTool(注:运行时可把 ragspine RAG 插为 Tool)
  orchestration/coordinator.py  Coordinator:顺序 / 并行跑多 agent、保序收集结果
  protocol/mcp/seam.py      McpClient / McpServer 协议 + OfflineMcpStub(离线回环)+ 延迟真实 SDK
  protocol/a2a/seam.py      A2AAgent 协议 + OfflineA2AStub(离线回环)+ 延迟真实 SDK
  conformance.py            本包绑定的不变量包(AGENT_INVARIANTS / TOOL_INVARIANTS)
```
