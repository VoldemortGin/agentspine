...
.PHONY: install
install: ## 建 venv 并可编辑安装:先装兄弟薄核 corespine,再装 agentspine[dev]
	uv venv $(VENV)
...
lint: ## ruff 静态检查(风格 + import 序 + 死代码)
	$(PYTHON) -m ruff check src/agentspine tests examples

.PHONY: fmt
fmt: ## ruff 自动格式化
	$(PYTHON) -m ruff format src/agentspine tests examples
